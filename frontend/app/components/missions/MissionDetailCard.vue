...
<template>
  <div class="group flex items-start justify-between gap-4 border border-border bg-card p-4 transition-colors hover:border-primary">
    <div class="min-w-0 flex-1 space-y-4">
      ...
      <div v-if="groups.length" class="space-y-3">
        <div v-for="g in groups" :key="g.name" class="border border-border bg-background p-3">
          <div class="flex items-center gap-2">
            <span class="font-serif text-sm font-semibold text-foreground">{{ g.name }}</span>
          </div>
          <div v-if="g.skills.length" class="mt-3 flex flex-wrap gap-2">
            <div
              v-for="skill in g.skills"
              :key="skill.name"
              class="flex items-center gap-2 border border-border bg-muted/40 px-2.5 py-1.5"
            >
              <span class="text-xs font-medium text-foreground">{{ skill.name }}</span>
              <span class="font-mono text-[10px] tracking-tight text-primary">{{ levelStars(skill.level) }}</span>
            </div>
          </div>
