...
<template>
  <div class="p-8 max-w-4xl">
    <div class="flex items-center justify-between mb-6">
      ...
      <Button v-if="!showNewForm" size="sm" @click="startCreate">
        <Icon name="lucide:plus" class="w-4 h-4 mr-1" /> Nouveau projet
      </Button>
    </div>

    <!-- Formulaire de création inline -->
    <div v-if="showNewForm" class="border border-primary/40 rounded-lg p-5 mb-4 bg-accent/10">
      <h2 class="font-semibold text-sm mb-4 text-muted-foreground uppercase tracking-wide">Nouveau projet</h2>
      <ContentMissionForm @submit="handleCreate" @cancel="closeForm" />
    </div>

    <div v-if="!missions?.length && !showNewForm" class="text-sm text-muted-foreground py-12 text-center">
      Aucun projet perso encore.
      <button @click="startCreate" class="text-primary underline ml-1">Créer le premier</button>
    </div>

    <div class="space-y-3">
      <template v-for="mission in missions" :key="mission.slug">
        <!-- Mode édition inline -->
        <div v-if="editingSlug === mission.slug" class="border border-primary/40 rounded-lg p-5 bg-accent/10">
