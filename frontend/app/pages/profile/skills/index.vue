  ...
  <div class="p-8 max-w-5xl space-y-6">
    ...
    <div v-for="(domainSkills, domainName) in grouped" :key="domainName" class="space-y-2">
      <div class="flex items-center gap-2">
        ...
        <span class="rounded border border-border px-1.5 py-0.5 text-xs text-muted-foreground">{{ domainSkills.length }}</span>
      </div>
      ...
      <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        <template v-for="skill in domainSkills" :key="skill.slug">
          ...
          <div
            ...
            class="rounded border border-primary/40 bg-accent/10 p-4 sm:col-span-2 lg:col-span-3"
          >
            <SkillsSkillForm
              :initial="skill"
              :domains="domains ?? []"
